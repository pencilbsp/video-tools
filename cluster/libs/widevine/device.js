...
export class WidevineDevice {
    constructor(bytes) {
        ...
        this.client_id_bytes = this._raw_bytes.subarray(
            11 + this.private_key_len,
            11 + this.private_key_len + this.client_id_len,
        );
    ...
    get_name() {
        ...
        let name = `[${type}]`;
        if (client_info["company_name"]) name += ` ${client_info["company_name"]}`;
        if (client_info["model_name"]) name += ` ${client_info["model_name"]}`;
        if (client_info["product_name"]) name += ` ${client_info["product_name"]}`;
        if (root_cert.systemId) name += ` (${root_cert.systemId})`;
